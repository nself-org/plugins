...
// sharp's 0.35 ESM type defs (dist/index.d.mts) dropped the old CJS
// `declare namespace sharp { ... }` merge that let call sites reference
// `sharp.OverlayOptions` / `sharp.Sharp` off the default import. `Sharp` and
// `composite()`'s parameter type are now plain named exports instead, so
// derive the element type from the method signature via the named `Sharp`
// import rather than the no-longer-resolvable namespace path.
type CompositeOverlay = Parameters<Sharp['composite']>[0][number];
...
export async function generateSpriteSheet(
  ...
  const compositeInputs: CompositeOverlay[] = [];
