...
func InitSchema(ctx context.Context, pool *pgxpool.Pool) error {
	stmts := []string{
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS idx_meetings_events_google_id ON meetings_events(source_account_id, google_event_id) WHERE google_event_id IS NOT NULL`,
		`CREATE UNIQUE INDEX IF NOT EXISTS idx_meetings_events_outlook_id ON meetings_events(source_account_id, outlook_event_id) WHERE outlook_event_id IS NOT NULL`,
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS idx_meetings_external_calendars_unique ON meetings_external_calendars(user_id, provider, source_account_id)`,
