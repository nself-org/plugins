...
// plugin never writes ~/.nself/auth.json. Only `nself claw session` uses
// this — the rest of the claw family authenticates against the claw server
// itself via NSELF_CLAW_API_KEY / ~/.nself/claw/config.yaml (see
// cmd/claw_config.go), an entirely separate credential.
package auth
