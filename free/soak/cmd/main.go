// Purpose: entry point for the standalone nself-soak binary.
//
// Inputs: os.Args via cobra's rootCmd.Execute().
//
// Outputs: process exit code — 0 on success, 1 on any cobra/runtime error.
// soak abort's own idempotent/cancel cases return nil (see abort.go), so this
// binary only ever exits 0 or 1, matching the pre-extraction command.
//
