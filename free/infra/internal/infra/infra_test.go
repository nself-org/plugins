...
func TestPlanErrorsWithoutTerraform(t *testing.T) {
	t.Setenv("PATH", t.TempDir())
	// context.Background() rather than t.Context(): testing.T.Context arrived in
	// Go 1.24 and this module targets 1.22, which `go vet` enforces from the go
	// directive even though the 1.26 toolchain compiles it. Plan returns as soon
	// as it fails to find terraform, so there is nothing for a cancellable
	// context to cancel.
