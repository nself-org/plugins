// Package tui provides the handful of terminal-output helpers the waf
// plugin needs to reproduce the exact look of `nself waf` from before
// extraction (CLI-R11).
//
...
// internal/ui that cmd/waf.go actually calls: a boxed command header, and
// the success/info/warn message lines. Colors and icons are copied
// byte-for-byte from internal/ui/{colors,icons}.go so output is unchanged
// for a user upgrading from the in-core command to the plugin.
//
