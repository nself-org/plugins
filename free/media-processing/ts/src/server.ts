...
export async function createServer(config?: Partial<Config>) {
  ...
  // Stop the queue processor interval and disconnect DB when server closes
  // (ensures the event loop drains cleanly in tests). Previously only
  // db.disconnect() ran here — the untracked setInterval(..., 5000) queue
  // processor below had no handle reference and was never cleared, keeping
  // the process alive indefinitely after app.close() in tests.
