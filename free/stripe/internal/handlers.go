...
// from STRIPE_ALLOWED_ORIGINS (csv). Wildcard is never accepted.
...
// corsMiddleware enforces explicit per-origin CORS. Credentials are only
// emitted alongside an echoed, allowlisted origin — never with a wildcard.
