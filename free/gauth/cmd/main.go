// Purpose: entry point for the standalone nself-gauth binary.
//
// Inputs: os.Args via cobra's rootCmd.Execute().
//
// Outputs: process exit code — 0 on success, 1 on any cobra/runtime error,
// matching the pre-extraction command (gauth's RunE functions only ever
// return nil or a plain error, never a custom exit code).
//
