// Purpose: entry point for the standalone nself-audit binary.
//
// Inputs: os.Args via cobra's rootCmd.Execute().
//
// Outputs: process exit code — 0 on success, 1 on a cobra/runtime error.
// docs.go's own RunE returns a non-nil error when findings are present
// (matching the pre-extraction "exit 1 = findings" contract), so this
// binary only ever exits 0 or 1.
//
