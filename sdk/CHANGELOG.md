# Changelog

All notable changes to plugin-sdk-go are documented here.

Format follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/).
Versions follow [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

---

## [Unreleased]

### Fixed
- **`ensureHealthRoutes` only answered `GET` on `/healthz`, `/health`, `/readyz`, not `HEAD`**: the nSelf CLI's docker-compose healthcheck template probes with `wget --spider`, which sends a `HEAD` request, not `GET`. Every plugin built on `sdk.NewServer` therefore had no route for `HEAD /health` — chi's router answered `405 Method Not Allowed` for the unmatched method — so the container healthcheck failed (`wget: server returned error: HTTP/1.1 405 Method Not Allowed`) and `docker compose ps` reported the plugin permanently unhealthy even though `GET /health` returned `200 {"status":"ok"}`. First observed on `free/cron` in the nself golden-path E2E, 2026-09-22. `ensureHealthRoutes` now registers a `HEAD` handler alongside `GET` for each liveness path, independently checked via chi's `Match` so a plugin that registers its own `HEAD` (or `GET`) route for a path keeps that handler; the default `HEAD` handler returns a bare `200` with no body (a `HEAD` response must not have one) so it agrees with both a real `net/http.Server` and `httptest.ResponseRecorder`. Re-shipped to all 23 consumers (17 vendored, 6 embedded), same rollout mechanism as the two entries below.
- **`server` package (`server.New`)**: `GET /health` now serves as an alias of `GET /healthz` (same handler, same JSON body). The nSelf CLI's docker-compose healthcheck convention (`internal/compose/custom_services.go`) probes `/health`, and every plugin's `docker-compose.plugin.yml` + `plugin.json` documents `/health` as the health endpoint — but `server.New` only ever registered `/healthz` and `/readyz`, so any consumer would 404 on the container healthcheck.
- **`sdk.NewServer` (root package shim, `shims.go`)**: this is the constructor most plugins actually call (`sdk.NewServer(port)`, not `server.New(Options)`), and it previously returned a bare `chi.Router` with **no routes at all** — not `/healthz`, not `/health`, not `/readyz`. Every plugin built on it (`notify`, `cron`, and 19 others — `backup`, `content-acquisition`, `content-progress`, `donorbox`, `feature-flags`, `github-runner`, `invitations`, `link-preview`, `mdns`, `mlflow`, `notifications`, `paypal`, `push`, `search`, `shopify`, `subtitle-manager`, `tokens`, `torrent-manager`, `vpn`, `webhooks`) shipped with no health route whatsoever, so their container healthchecks 404'd and `docker compose ps` reported them permanently unhealthy even though the service was live. `NewServer` now pre-registers `/healthz`, `/health`, and `/readyz` against a shared liveness handler (callers may still override any of them on the returned `Router()`). Root-caused via the nself CLI golden-path E2E, 2026-09-21 (defect first observed on `notify`/`cron`).
- **`sdk.NewServer` panicked on the standard plugin pattern** (introduced by the previous entry, PR #104): pre-registering `/healthz`, `/health`, `/readyz` inside `NewServer` meant every plugin's own `r := srv.Router(); r.Use(sdk.Recovery); ...` call now hit chi's `panic: chi: all middlewares must be defined before routes on a mux`, since those three routes already existed on the mux before `Use` ran. Seen live in the nself golden-path E2E for `notify` (`cmd/main.go:38`) and `cron` (`cmd/main.go:56`). #104's unit test never called `Use` after `NewServer`, so it never caught the ordering violation. Fixed by deferring route registration out of `NewServer` and into `ListenAndServe` (via a new internal `ensureHealthRoutes` helper), which runs after every plugin's own `Use`/route calls; `ensureHealthRoutes` also uses chi's `Match` to skip any path the plugin already registered itself, so an explicit plugin `/health` route still wins. Re-shipped to all 23 consumers (17 vendored, 6 embedded), same as #104's original rollout.

## [0.1.0] - 2026-04-23

### Added

Initial public release. Extracted from nSelf private plugin infrastructure.

- `plugin` — `Plugin` interface, `Info` struct, `Base` embed, `Validate()`
- `logger` — JSON `slog.Logger` factory with `plugin` + `version` attrs
- `config` — env-driven config loader with validation helpers
- `server` — chi router with `/healthz`, `/readyz`, `/metrics`, `/version`
- `metrics` — shared Prometheus registry + per-plugin request/error counters
- `license` — offline license cache, grace period, skip-verify dev flag
- `httpx` — HTTP client with retries, timeouts, propagated request-ID header
- `db` — `pgxpool` connect/health/migration helpers
- `tracing` — OpenTelemetry tracer + request-ID middleware
- `middleware` — request-ID injection, common validation helpers
- `costmeter` — shared cost accounting for AI/inference plugins
- `identity` — Ed25519 per-plugin keypair + request signing/verification
- `testing` — test harness: `StubUpstream`, `DoJSONRequest`, `FetchMetrics`, `AssertHealthEndpoints`
- `devkit/cmd/new-plugin` — scaffolding generator for new plugin projects
- `compatibility.go` — `CheckMinSDK` and `CheckCLICompat` runtime guards
- `doc.go` — `Version` constant (`"0.1.0"`)
- CI workflow: test (Go 1.23/1.24), govulncheck, private-import-path security check

### Notes

- Requires Go 1.23 or later.
- Targets nSelf CLI v1.0.9 and newer.
- Zero dependencies on `plugins-pro` private code.

[0.1.0]: https://github.com/nself-org/cli/sdk/go/releases/tag/v0.1.0


## [0.1.1] - 2026-04-25

### Fixed
- **`CheckCLICompat`**: updated minimum compatible CLI version constant to `v1.0.12`.
- **`registry` package**: plugin registry endpoint defaulted to `plugins.nself.org` — now validates TLS cert fingerprint on first connection.

### Changed
- SPORT F01 plugin count updated to 25 free + 87 paid (87 confirmed via plugins-pro schema validator run in P96).
